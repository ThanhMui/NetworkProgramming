...
//               listSecretKeys  = (HashMap) deserialize(receivePacket.getData());
				...
				if (mapReceiveObj.containsKey("send1") && mapReceiveObj.size() == 1) {
					...
					System.out.println("Server sent " + listPublicKeys.get("publicKey") + " to "
							+ receivePacket.getAddress() + " from port " + datagramSocket.getLocalPort());
					...
					System.out.println(
							"Server received: " + tmpEncrypt + " from " + receivePacket.getAddress().getHostAddress()
					...
					if (result.contains("$city")) {
						...
						if (getInfoCityFulls.size() > 0) {
							...
							for (City city : getInfoCityFulls) {
								...
								byte[] strEncryptClouds = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(city.getClouds()).getBytes());
								byte[] strEncryptCountry = Encrypt.AESUtils.encrypt(secretKey,
										city.getCountry().getBytes());
								byte[] strEncryptWeather = Encrypt.AESUtils.encrypt(secretKey,
										city.getDescriptionWeather().getBytes());
								byte[] strEncryptIdCountry = Encrypt.AESUtils.encrypt(secretKey,
										city.getIdCountry().getBytes());
								byte[] strEncryptIdProvince = Encrypt.AESUtils.encrypt(secretKey,
										city.getIdProvince().getBytes());
								byte[] strEncryptLatitude = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(city.getLatitude()).getBytes());
								byte[] strEncryptLong = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(city.getLongitude()).getBytes());
								byte[] strEncryptMax = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(city.getMax_Temperature()).getBytes());
								byte[] strEncryptMin = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(city.getMin_Temperature()).getBytes());
								byte[] strEncryptTemp = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(city.getTemperature()).getBytes());
								byte[] strEncryptProvince = Encrypt.AESUtils.encrypt(secretKey,
										city.getNameProvince().getBytes());
								byte[] strEncryptPopu = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(city.getPopulation()).getBytes());
								byte[] strEncryptSpeed = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(city.getSpeedWind()).getBytes());
								byte[] strEncryptTimeZ = Encrypt.AESUtils.encrypt(secretKey,
										city.getTimezoneId().getBytes());
...
//	                sendData = serialize("test");
						...
						if (getInfoCountryFulls.size() > 0) {
							for (CountryAll country : getInfoCountryFulls) {
								byte[] strEncryptTimeZ = Encrypt.AESUtils.encrypt(secretKey,
										country.getAlpha2Code().getBytes());
								byte[] strEncryptCap = Encrypt.AESUtils.encrypt(secretKey,
										country.getCapital().getBytes());
								byte[] strEncryptClo = Encrypt.AESUtils.encrypt(secretKey,
										country.getClouds().getBytes());
								byte[] strEncryptCodeCoun = Encrypt.AESUtils.encrypt(secretKey,
										country.getCountryCode().getBytes());
								byte[] strEncryptCurre = Encrypt.AESUtils.encrypt(secretKey,
										country.getCurrencies().getBytes());
								byte[] strEncryptDate = Encrypt.AESUtils.encrypt(secretKey,
										country.getDatetime().getBytes());
								byte[] strEncryptFlag = Encrypt.AESUtils.encrypt(secretKey,
										country.getFlag().getBytes());
								byte[] strEncryptGeo = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(country.getGeonameId()).getBytes());
								byte[] strEncryptHum = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(country.getHumidity()).getBytes());
								byte[] strEncryptLanguge = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(country.getLanguages()).getBytes());
								byte[] strEncryptLat = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(country.getLatitude()).getBytes());
								byte[] strEncryptLong = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(country.getLongtitude()).getBytes());
								byte[] strEncryptName = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(country.getName()).getBytes());
								byte[] strEncryptNeigh = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(country.getNeighbours()).getBytes());
								byte[] strEncryptPopu = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(country.getPopulation()).getBytes());
								byte[] strEncryptTemp = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(country.getTemperature()).getBytes());
								byte[] strEncryptSpeed = Encrypt.AESUtils.encrypt(secretKey,
										String.valueOf(country.getWindSpeed()).getBytes());
								byte[] strEncryptDesc = Encrypt.AESUtils.encrypt(secretKey,
										country.getWeatherCondition().getBytes());
...
//						countrySearch = getNameCountrys(result).get(0);
						String covidSearch = getNameCitys(result).get(0);
						String[] str = covidSearch.split(",");
						String country = str[0];
						String dateStart = str[1];
						String dateEnd = str[2];
						List<CovidInfoModel> listCovid = InfomationCovid.getDataCovid2(country, dateStart, dateEnd);
						List<byte[]> listEncrypt = new ArrayList<>();

						byte[] encryptList = serialize(listCovid);
						...
						dataCovid = InformationCovidTop.getData22();
						List<byte[]> listEncrypt = new ArrayList<>();
